  ...
  return (
    <Card>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
          <div className="flex items-center gap-2 text-emerald-700">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider">Entradas</p>
          </div>
          <p className="mt-2 text-2xl font-bold text-emerald-700">{entryUnits.toLocaleString()}</p>
          <p className="text-xs font-bold text-emerald-600">{money(entryCost)} costo estimado</p>
        </div>
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4">
          <div className="flex items-center gap-2 text-rose-700">
            <ArrowUpFromLine size={18} />
            <p className="text-[10px] font-bold uppercase tracking-wider">Salidas</p>
          </div>
          <p className="mt-2 text-2xl font-bold text-rose-700">{exitUnits.toLocaleString()}</p>
          <p className="text-xs font-bold text-rose-600">{money(exitCost)} costo estimado</p>
        </div>
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
          <div className="flex items-center gap-2 text-amber-700">
            <AlertCircle size={18} />
            <p className="text-[10px] font-bold uppercase tracking-wider">Ajustes</p>
          </div>
          <p className="mt-2 text-2xl font-bold text-amber-700">{adjustmentUnits.toLocaleString()}</p>
      ...
      <div className="mt-5 rounded-2xl border border-[var(--app-border)] bg-[var(--app-bg-subtle)] p-4">
        <h4 className="text-sm font-bold text-[var(--app-text)]">Lectura ejecutiva</h4>
          ...
          <tbody>
            {inventoryMovements.slice(0, 8).map((movement, index) => (
              <tr key={`${movement.productName}-${movement.createdAt}-${index}`}>
                ...
                <td>
                  <span className="rounded-lg bg-[var(--app-primary-soft)] px-2 py-1 text-[10px] font-bold uppercase text-[var(--app-primary)]">
                    {movementLabel[movement.movementType] || movement.movementType}
                  </span>
                </td>
                <td className="text-right font-bold tabular-nums">{num(movement.quantity).toLocaleString()}</td>
                <td className="text-right font-bold tabular-nums">{money(movement.totalCost)}</td>
              </tr>
            ))}
            {inventoryMovements.length === 0 && (
              <tr>
                <td colSpan="5" className="py-8 text-center text-xs font-bold uppercase text-[var(--app-text-muted)]">
