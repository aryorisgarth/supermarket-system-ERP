...
import { Receipt, Loader2, FileText, Undo2, XCircle } from 'lucide-react';
...
const AdminBillingDetailModal = ({
  ...
  return (
    <ResponsiveModal
      ...
      onClose={onClose}
      icon={Receipt}
      title={`Desglose - Factura ${sale.invoiceNumber}`}
      subtitle={`Cajero: ${sale.seller?.fullName || 'Sistema'} | Fecha: ${new Date(sale.saleDate).toLocaleString()}`}
      initialSize="lg"
      sizeOptions={['md', 'lg', 'xl']}
      headerClassName="bg-gradient-to-r from-primary to-indigo-600 text-white"
      footer={
