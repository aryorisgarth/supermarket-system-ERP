...
import { BookOpen, Check, Copy, Loader2, Search, Tag, X, Layers, Box } from 'lucide-react';
...
import { filterPluProducts } from '../../utils/pluProductUtils';

const PosQuickCodesPanel = ({ open, products: productsProp = [], onClose, onSelectCode }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [activeProducts, config] = await Promise.all([
          ProductService.getAllActive(),
          ScaleConfigService.getConfig().catch(() => null),
        ]);
        if (cancelled) return;
        setLoadedProducts(normalizeProductList(activeProducts));
        if (config) setScaleConfig(config);
      } catch (err) {
        console.warn('No se pudo cargar catálogo PLU:', err);
        if (!cancelled) setLoadedProducts(normalizeProductList(productsProp));
      } finally {
  ...
  const sourceProducts = loadedProducts.length > 0 ? loadedProducts : productsProp;

  const pluProducts = useMemo(
    () => filterPluProducts(sourceProducts, scaleConfig),
    [sourceProducts, scaleConfig],
  );
        ...
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-4 scrollbar-thin scrollbar-thumb-gray-300">
          {loading ? (
            <div className="text-center py-10 text-gray-500">
              ...
              <p className="text-xs font-black uppercase tracking-wider">Cargando PLU…</p>
            </div>
          ) : grouped.length === 0 ? (
            <div className="text-center py-10 text-gray-500">
              ...
              <p className="mt-2 text-[11px] font-medium normal-case tracking-normal text-gray-400 px-4">
                Verifique que existan productos con código corto (ej. 4011) o unidad LB/KG en inventario.
              </p>
            </div>
          ) : (
